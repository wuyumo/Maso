...
enum PlanShareCodec {
    ...
    /// source/rationale 也一并保留 → 导入的 AI/Classics 计划徽章 + 理由行不丢 (跟 DataStore.savePlan
    /// 重 id 时同约定); 只有 id/时间戳/autoGenerated 这些"归属"字段按接收方语义重置.
    private static func remapped(_ plan: Plan) -> Plan {
        ...
        return Plan(
            ...
            source: plan.resolvedSource,   // 保留 AI/Classics 来源 → 导入后徽章不丢 (原 id 前缀会随新 id 丢, 故落成显式值)
            rationale: plan.rationale      // 保留 AI 理由行 → 导入后卡上仍显示
        )
    }
