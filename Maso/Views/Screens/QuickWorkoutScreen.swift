            ...
            Group {
                if step == 1 {
                    ...
                    QuickExerciseStep(
                        ...
                        aiOrder: $aiOrder,
                        exById: data.exById,
                        // 主"选动作"入口 — 用 userLibrary (含 custom + adopted niche, 排除未采纳 niche).
                        // 跟 ExercisePickerSheet 同 source. Smart pick / 列表不会出现 Foam Roll /
                        // Battle Rope / Hip Abduction Machine 等怪东西, 除非用户主动采纳.
                        allExercises: data.userLibrary,
                        onStart: handleStart
                    )
                }
            }
...
private struct QuickMuscleStep: View {
    ...
    private var anySelectedJustTrained: Bool {
        ...
        for m in expanded {
            if MuscleStatusCompute.tierFor(muscle: m, fatigueMap: fatigueMap) == .fatigued {
                return true
            }
        }
        return false
